...
(function() {
  var RequestIO, RoomIO, async, cookieParser, express, expressLayer, expressSession, http, https, initRoutes, io, key, listen, middleware, session, sessionConfig, value, _, redis;
  ...
  express.session = function(options) {
    ...
    if ((_ref1 = options.store) == null) {
      ...
      // Configure support for Redis
      if (options.storeType = 'redis') {
        var RedisStore = redis(session);
        option.store = new RedisStore(options.redisOptions ? options.redisOptions : {});

        delete options.storeType;
        delete options.redisOptions;
      }
    }
